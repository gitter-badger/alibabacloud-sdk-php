...
class CompatibilityTest {
	...
	public function __construct() {
		...
		$title       = 'Alibaba Cloud SDK for PHP Compatibility Test';
		if ($this->isCli) {
			$rep           = str_repeat('=', strlen($title));
			$this->lines[] = "{$rep}\n{$title}\n{$rep}";
		} else {
			$this->lines[] = sprintf('<style type="text/css">%s %s</style>',
	...
	public function iniCheck($info, $setting, $expected, $required = true, $help = null) {
		...
		$message = sprintf('%s in %s is currently set to %s but %s be set to %s.',
						   $setting,
						   php_ini_loaded_file(),
						   var_export($current, true),
						   $required ? 'must' : 'should',
						   var_export($expected, true)) . ' ' . $help;
...
$c->addRequire('Ensuring that the version of PHP is >= 5.5.0',
	function() {
		return version_compare(phpversion(), '5.5.0', '>=');
	},
			   'You must update your version of PHP to 5.5.0 to run the Alibaba Cloud SDK for PHP');

$c->iniCheck('Ensuring that detect_unicode is disabled',
			 'detect_unicode',
			 false,
			 true,
...
if (extension_loaded('suhosin')) {
	$c->addRequire('Ensuring that phar files can be run with the suhosin patch',
		function() {
...
if (function_exists('curl_version')) {
	$c->addRequire('Ensuring that cURL can send https requests',
		function() {
...
$c->addRequire('Ensuring that file_get_contents works',
	function() {
...
$c->addRecommend('Checking if you are running on a 64-bit platform',
	function() {
...
$c->check('Ensuring that date.timezone is set',
	function() {
		return (bool)ini_get('date.timezone');
	},
		  'The date.timezone PHP ini setting has not been set in ' . php_ini_loaded_file(),
		  false);

if (extension_loaded('xdebug')) {
	$c->addRecommend('Checking if Xdebug is installed',
		function() {
			return false;
		},
