...
This guide focuses on basic usage patterns of the **Alibaba Cloud SDK for PHP**. This
guide assumes that you have already :doc:`downloaded and installed the SDK
<installation>` and retrieved your `Alibaba Cloud access keys
<http://aliyun.com/developers/access-keys/>`_.
...
-------------------------- -----------------------------------------------------
Using the Phar             ``require '/path/to/aliyun.phar';``
-------------------------- -----------------------------------------------------
Using the Zip              ``require '/path/to/aliyun-autoloader.php';``
========================== =====================================================
...
for the Alibaba Cloud service you want to interact with. Client objects have methods that
...
``ALIBABA_CLOUD_ACCESS_KEY_ID`` and ``ALIBABA_CLOUD_SECRET_ACCESS_KEY``), an
:ref:`Alibaba Cloud credentials INI file <credential_profiles>` in your HOME
directory, Alibaba Cloud Identity and Access Management (IAM)
...
The ``AlibabaCloud\Sdk`` class acts as a client factory and is used to manage shared
...
``AlibabaCloud\Sdk`` class. These options are then applied to each client constructor.
...
.. code-block:: php
    ...
    $sdk = new AlibabaCloud\Sdk($sharedConfig);

    // Create an Alibaba S3 client using the shared configuration data.
    $client = $sdk->createS3();
...
a client object. For example, to perform the Alibaba S3 `PutObject operation
<http://docs.aliyun.com/AlibabaS3/latest/API/RESTObjectPUT.html>`_, you must
call the ``AlibabaCloud\S3\S3Client::putObject()`` method.

.. code-block:: php

    // Use an AlibabaCloud\Sdk class to create the S3Client object.
...
Executing an successful operation will return an ``AlibabaCloud\Result`` object. Instead
...
In the following example, the ``AlibabaCloud\S3\S3Client`` is used. If there is an
error, the exception thrown will be of the type ``AlibabaCloud\S3\Exception\S3Exception``.
All service specific exceptions thrown by the SDK extend from the
``AlibabaCloud\Exception\AlibabaCloudException`` class. This class contains useful information
...
.. code-block:: php
    ...
    try {
        ...
        // This catches the more generic AlibabaCloudException. You can grab information
        // from the exception using methods of the exception object.
        echo $e->getAlibabaCloudRequestId() . "\n";
        echo $e->getAlibabaCloudErrorType() . "\n";
        echo $e->getAlibabaCloudErrorCode() . "\n";
    }
