...
Some Alibaba Cloud service operations are paginated and respond with truncated results.
For example, Alibaba S3's ``ListObjects`` operation only returns up to 1000
...
object using ``foreach`` to get individual ``AlibabaCloud\Result`` objects.
...
``AlibabaCloud\ResultPaginator`` class. This class implements PHP's native ``Iterator``
...
callback the the ``each()`` method of an ``AlibabaCloud\ResultPaginator``. The callback
