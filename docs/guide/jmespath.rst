...
extract elements from a JSON document. The Alibaba Cloud SDK for PHP has a dependency on
...
exposes JMESPath searching on ``AlibabaCloud\ResultInterface`` and
``AlibabaCloud\ResultPaginator``.
...
JMESPath is supported in the `Alibaba Cloud CLI <http://aliyun.com/cli/>`_.
Expressions you write for CLI output are 100% compatible with expressions
written for the Alibaba Cloud SDK for PHP.

Extracting data from results
----------------------------

The ``AlibabaCloud\ResultInterface`` interface has a ``search($expression)`` method that
...
separate Alibaba EC2 instances.
...
.. code-block:: json

    {
        ...
        "@metadata": {
            "statusCode": 200,
            "effectiveUri": "https:\/\/ec2.us-west-2.Alibabaaliyun.com",
            "headers": {
...
As you know from the :doc:`paginators` guide, ``AlibabaCloud\ResultPaginator`` objects
are used to yield results from a pageable API operation. The SDK allows you to
extract and iterate over filtered data from ``AlibabaCloud\ResultPaginator`` objects
