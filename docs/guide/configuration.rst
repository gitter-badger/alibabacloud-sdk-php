...
The following example shows how to pass options into an Alibaba S3 client
constructor.
...
By default, the SDK will use an instance of ``AlibabaCloud\Api\FileSystemApiProvider``
that loads API files from the ``src/data`` folder of the SDK.

credentials
~~~~~~~~~~~

:Type: ``array|AlibabaCloud\CacheInterface|AlibabaCloud\Credentials\CredentialsInterface|bool|callable``
...
.. code-block:: php
    ...
    $s3 = new AlibabaCloud\S3\S3Client([
        'version'     => 'latest',
        'region'      => 'us-west-2',
        'credentials' => [
            'key'    => 'abc',
...
Pass an ``AlibabaCloud\Credentials\CredentialsInterface`` object to use a specific
credentials instance.

.. code-block:: php

    $credentials = new AlibabaCloud\Credentials\Credentials('key', 'secret');
...
Pass an instance of ``AlibabaCloud\CacheInterface`` to cache the values returned by the
...
scrub_auth (bool)
    ...
    messages (meaning your Alibaba Cloud Access Key ID and signature will be passed
...
.. tip::
    ...
    The debug output is extremely useful when diagnosing issues in the Alibaba Cloud
...
custom endpoint (e.g., a local version of Alibaba S3 or
`Alibaba DynamoDB Local <http://docs.aliyun.com/Alibabadynamodb/latest/developerguide/Tools.DynamoDBLocal.html>`_).

Here's an example of connecting to Alibaba DynamoDB Local:
...
See http://docs.aliyun.com/general/latest/gr/rande.html for a list of
...
``AlibabaCloud\ResultInterface`` object or rejected with an
``AlibabaCloud\Exception\AlibabaCloudException``. A handler does not accept a next handler as it
is terminal and expected to fulfill a command. If no handler is provided, a
default Guzzle handler will be utilized.

You can use the ``AlibabaCloud\MockHandler`` to return mocked results or throw mock
...
.. code-block:: php
    ...
    $mock->append(function (CommandInterface $cmd, RequestInterface $req) {
        return new AlibabaCloudException('Mock exception', $cmd);
    });
...
You can connect to an Alibaba Cloud service through a proxy using the ``proxy`` option.
...
up-front. For example, this option is relied upon in the Alibaba S3 stream
...
``AlibabaCloud\WrappedHttpHandler`` object.
...
the Alibaba Cloud credentials file in your HOME directory. This setting overrides the
``ALIBABA_CLOUD_PROFILE`` environment variable. Note: Specifying "profile" will cause
the "credentials" key to be ignored.

.. code-block:: php

    // Use the "production" profile from your credentials file.
    $ec2 = new AlibabaCloud\Ec2\Ec2Client([
        'version' => '2014-10-01',
        'region'  => 'us-west-2',
        'profile' => 'production'
    ]);
...
Region to connect to. See http://docs.aliyun.com/general/latest/gr/rande.html
...
The following example disables retries for the Alibaba DynamoDB client.
...
using a client provided by the SDK (i.e., ``AlibabaCloud\S3\S3Client``). This option
...
service name, and region, and returns a ``AlibabaCloud\Signature\SignatureInterface``
...
``AlibabaCloud\Signature\SignatureProvider`` class that can be used to create customized
signature providers.
...
The following examples show how to configure an Alibaba S3 client to use
`signature version 4 <http://docs.aliyun.com/general/latest/gr/signature-version-4.html>`_:
...
service. For example, when using Alibaba S3, you can lock your API version to
``2006-03-01``.
