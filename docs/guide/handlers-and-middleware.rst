...
**middleware**. Each SDK client class owns an ``AlibabaCloud\HandlerList`` instance that
...
``setHandler`` method of an ``AlibabaCloud\ClientInterface``:
...
The SDK uses an ``AlibabaCloud\HandlerList`` to manage the middlewares and handlers used
...
The ``AlibabaCloud\Middleware::mapCommand`` middleware is useful when you need to modify
...
``AlibabaCloud\CommandInterface`` object and returns an ``AlibabaCloud\CommandInterface`` object.
...
The ``AlibabaCloud\Middleware::mapRequest`` middleware is useful when you need to modify
...
The ``AlibabaCloud\Middleware::mapResult`` middleware is useful when you need to modify
the result of a command execution. The ``mapResult`` function accepts a
callable that accepts an ``AlibabaCloud\ResultInterface`` argument and returns an
``AlibabaCloud\ResultInterface`` object.
...
.. code-block:: php
    ...
    use AlibabaCloud\History;
    use AlibabaCloud\Middleware;

    $ddb = new AlibabaCloud\DynamoDb\DynamoDbClient([
        'version' => 'latest',
        'region'  => 'us-west-2'
    ]);
...
An ``AlibabaCloud\History`` history container will store 10 entries by default before
...
.. code-block:: php
    ...
    // You can get the last AlibabaCloud\CommandInterface that was executed. This method
    ...
    // You can get the last return value (an AlibabaCloud\ResultInterface or Exception).
...
``tap`` function accepts a callable that accepts the ``AlibabaCloud\CommandInterface``
...
A handler is simply a function that accepts an ``AlibabaCloud\CommandInterface`` object
and ``Psr\Http\Message\RequestInterface`` object and returns a
``GuzzleHttp\Promise\PromiseInterface`` that is fulfilled with an
``AlibabaCloud\ResultInterface`` or rejected with an ``AlibabaCloud\Exception\AlibabaCloudException``.
