==========================
Alibaba S3 Transfer Manager
==========================

The PHP SDK Alibaba S3 transfer manager is used to upload entire directories to
an Alibaba S3 bucket and download entire buckets to a local directory.

Uploading a local directory to S3
---------------------------------

The ``AlibabaCloud\S3\Transfer`` object is used to perform transfers. The following
...
.. code-block:: php
    ...
    $manager = new \AlibabaCloud\S3\Transfer($client, $source, $dest);

    // Perform the transfer synchronously.
    $manager->transfer();

In this example, we created an Alibaba S3 client, created a ``Transfer`` object,
...
You can upload the local files to a "sub-folder" of a an Alibaba S3 bucket by
...
Downloading an Alibaba S3 bucket
-------------------------------

You can recursively download an Alibaba S3 bucket to a local directory on disk
by specifying the ``$source`` argument as an Alibaba S3 URI
...
You can include a key prefix in the Alibaba S3 URI after the bucket to download
...
``$source``
    ...
    to a local path on disk (e.g., ``/path/to/files``) or an Alibaba S3 bucket
    (e.g., ``s3://bucket``). The ``s3://`` URI may also contain a key prefix
    that can be used to only transfer objects under a common prefix.

    If the ``$source`` argument is an Alibaba S3 URI, then the ``$dest``
...
``$dest``
    ...
    argument is a local path on disk, then ``$dest`` must be an Alibaba S3
    bucket URI (e.g., ``s3://bucket``). If the ``$source`` argument is an
    Alibaba S3 bucket URI, then the ``$dest`` argument must be a local path on
    disk.
...
``before``
    ...
    have a function signature like ``function (AlibabaCloud\Command $command) {...}``.
...
typically be an instance of ``AlibabaCloud\Exception\AlibabaCloudException`` (though a value of
...
.. code-block:: php
    ...
    $uploader = new Transfer($s3Client, $source, $dest, [
        'before' => function (\AlibabaCloud\Command $command) {
