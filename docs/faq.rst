...
http://docs.aliyun.com/general/latest/gr/rande.html
...
Alibaba Cloud SDK for PHP does not correctly handle files larger than 2GB on a 32-bit
...
The recommended solution is to use a `64-bit Linux stack <http://aliyun.com/Alibaba-linux-ami/>`_,
such as the 64-bit Alibaba Linux AMI with the latest version of PHP installed.
...
middleware to the ``AlibabaCloud\HandlerList`` of an ``AlibabaCloud\CommandInterface`` or
``AlibabaCloud\ClientInterface``. The following example shows how to add an
``X-Foo-Baz`` header to a specific Alibaba S3 PutObject operation using the
``AlibabaCloud\Middleware::mapRequest`` helper method.
...
middleware to the ``AlibabaCloud\HandlerList`` of an ``AlibabaCloud\CommandInterface`` or
``AlibabaCloud\ClientInterface``. The following example shows how to add custom command
parameters to a command before it is sent, essentially adding default options.
This example uses the ``AlibabaCloud\Middleware::mapCommand`` helper method.
...
If you are seeing a ``AlibabaCloud\Exception\CredentialsException`` while while using
...
profile credentials, which are only available on configured Alibaba EC2
instances. If absolutely no credentials are provided or found, an
``AlibabaCloud\Exception\CredentialsException`` is thrown.

If you are seeing this error and you are intending to use instance profile
credentials, then you need to make sure that the Alibaba EC2 instance that the
...
``http`` access. Please see `regions and endpoints <http://docs.aliyun.com/general/latest/gr/rande.html>`_
...
command object to ``AlibabaCloud\Signature\S3SignatureV4::UNSIGNED_PAYLOAD``. Then PHP SDK will use it as
...
.. code-block:: php
    ...
    $params = [
        'Bucket' => 'foo',
        'Key'    => 'baz',
        'ContentSHA256' => AlibabaCloud\Signature\S3SignatureV4::UNSIGNED_PAYLOAD
    ];
...
.. code-block:: php
    ...
    $sns = new \AlibabaCloud\Sns\SnsClient([
        'region' => 'us-west-2',
        'version' => 'latest',
    ]);
    ...
    try {
        ...
        switch ($e->getAlibabaCloudErrorCode()) {
